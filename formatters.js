importScripts('/packages/planifica_file-encryption/lzw.js');
...
var Latin1Formatter = {
    ...
    stringify: function (cipherParams, opts) {
        ...
        // Format
        if (salt) {
            wordArray = CryptoJS.lib.WordArray.create([0x53616c74,
                0x65645f5f
    ...
    parse: function (str) {
        ...
        var ciphertext = CryptoJS.enc.Latin1.parse(str),
            salt;

        // Test for salt
        if (ciphertext.words[0] == 0x53616c74 && ciphertext.words[1] ==
            0x65645f5f) {
            // Extract salt
            salt = CryptoJS.lib.WordArray.create(ciphertext.words.slice(
                2, 4));
